...
-- Then create it with the correct column names (intentional typos preserved for compatibility)
...
SELECT
  1 AS id,
  (SELECT SUM(cs_documents_in_process) FROM public.customers WHERE is_active = true) AS total_in_proces, -- Typo preserved
  ...
  ) AS top_in_process) AS total_in_process_15, -- Typo preserved
  ...
  (SELECT SUM(cs_documents_other) FROM (
      ...
      LIMIT (SELECT topx FROM settings_value)
  ) AS top_other) AS total_other_15, -- Typo preserved
  ...
  (SELECT SUM(combined_total) FROM (
      ...
      LIMIT (SELECT topx FROM settings_value)
  ) AS top_total) AS total_15; -- Typo preserved
