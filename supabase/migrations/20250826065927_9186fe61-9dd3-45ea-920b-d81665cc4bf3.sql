-- Phase 1b: Refactor functions to SECURITY INVOKER to avoid bypassing RLS through views

-- get_cct_customers as SECURITY INVOKER
CREATE OR REPLACE FUNCTION public.get_cct_customers()
 RETURNS TABLE(id character varying, created_at timestamp with time zone, customer_name character varying, administration_name character varying, administration_mail character varying, source character varying, source_root character varying, cs_last_update timestamp with time zone, cs_documents_in_process integer, cs_documents_other integer, cs_documents_inbox integer, is_active boolean, cct_processed boolean, last_updated_by character varying)
 LANGUAGE sql
...
-- get_cct_stats as SECURITY INVOKER
